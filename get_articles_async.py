...
class article_obj:
    ...
    def __str__(self):
        return 'title: ' + self.title + ', words: ' + str(self.word_count)\
            + ', url: ' + self.url
...
class all_articles:
    ...
    async def populate_articles_list_helper(self, themes: list,
                                            websites: list):
    ...
    async def nytimes_search(self, session: aiohttp.ClientSession, theme: str):
        ...
        api_request_url = 'https://api.nytimes.com/svc/search/v2/a\
                           rticlesearch.json?q=theme&api-key='
        ...
        async with session.get(api_request_url) as response:
            ...
            for article in response_object['response']['docs']:
                ...
                try:
                    ...
                    url = article['web_url']
                    if word_count != 0 and article_headline != '' \
                            and url != '':
                        article_object = article_obj(article_headline,
                                                     word_count, url)
    ...
    async def medium_search(self, session: aiohttp.ClientSession, theme: str):
        ...
        async with session.get(api_request_url) as response:
            ...
            response_object_text = await response.text()
            response_json_string = response_object_text.split(
                    'APOLLO_STATE__ =')[1].split(
                            '</script><middlewarestate>')[0]
            # Turn the object into a python dictionary
            try:
            ...
            for key in response_object:
                if (key.startswith('Post:')):
                    ...
                    try:
                        ...
                        word_count = round(250 *
                                           response_object[key]['readingTime'])
                        url = response_object[key]['mediumUrl']
                        # If all the fields are not empty add
                        # the object you want
                        if word_count != 0 and \
                                article_headline != '' and url != '':
                            article_object = article_obj(article_headline,
                                                         word_count, url)
                            self.articles_list.append(article_object)
                    except KeyError as e:
    ...
    def provide_articles_by_reading_time(self, reading_time: int,
                                         reading_speed: int) \
            -> Tuple[list, int]:
        ...
        while (time < (reading_time - 5)) and (len(self.articles_list) != 0):
            ...
            time = time + ((self.articles_list[random_variable].word_count)
                           / reading_speed)
