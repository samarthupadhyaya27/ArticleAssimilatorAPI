...
class article_obj:
    def __init__(self, title, word_count, url):
        ...
        self.url = url
    def __str__(self):
        return "title: " + self.title + ", reading: " + self.word_count + ", url: " + self.url
class all_articles:
...
# print("AttributeError")
    ...
    def nytimes_search_by_themes(self, theme):
        ...
        url = "https://www.nytimes.com/search?dropmab=false&query=" + theme + "&sort=best&types=article"
        ...
        for article in article_body:
            ...
            article_body = webpage_2.find('section', class_='meteredContent css-1r7ky0e')
            text_string = ""
...
#setting each of the tags to a url
            levenshtein_distance_dict[a.text.lower()] = Levenshtein.distance(theme, a.text.lower())
#making a dictionary mapping the theme names to their Levenshtein distance
        min_distance_key = min(levenshtein_distance_dict, key=levenshtein_distance_dict.get)
...
#get a beautiful soup rendering of the page with articles relating to that theme
        articles = webpage_2.find_all('div', class_="cardChromeless u-marginTop20 u-paddingTop10 u-paddingBottom15 u-paddingLeft20 u-paddingRight20")
        ...
        if len(articles) > 0:
            for article in articles:
                try:
                    ...
                    reading_time = article.find('span', class_="readingTime").get('title')[0]
...
#Add the article tot he list of articles to present
            time = time + ((int(self.articles_list[random_variable].word_count)) / int(reading_speed))
